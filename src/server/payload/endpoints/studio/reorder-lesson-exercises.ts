 ...
 * a specified sibling. Studio uses this after calling the existing prod
 * `/api/exercises/:id/duplicate-exercise` endpoint (which appends the new
 * exercise at the end of the parent lesson's playlist) to snap the copy
 * back to the intuitive position — right below the source.
 *
 * Body: { movedExerciseId: string, insertAfterExerciseId: string }
 *
 * Access: admin or advanced content editor.
 *
 * @fileType api-route
 * @domain admin-studio
