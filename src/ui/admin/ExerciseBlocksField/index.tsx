 ...
 *
 *  Only `sectionRef` is supported today. Other block types (e.g. richText,
 *  contentPageRef) are schema-only additions in the future.
 */
function normalizeBlock(block: RawBlock): RawBlock | null {
  ...
  if (block.blockType === 'sectionRef') {
    const id = extractId(block.section)
    if (!id) return null
...
export const ExerciseBlocksField: React.FC<{ path: string }> = ({ path }) => {
  ...
  useEffect(() => {
    ...
    for (const block of blocks) {
      if (block.blockType === 'sectionRef') {
        const id = extractId(block.section)
        const title = extractTitle(block.section)
  ...
  useEffect(() => {
    ...
    for (const block of blocks) {
      if (block.blockType !== 'sectionRef') continue
      const id = extractId(block.section)
    ...
    for (const { id } of idsToFetch) {
      fetch(`/api/sections/${id}?depth=0`, { credentials: 'include' })
  ...
  const rows: ResolvedRow[] = useMemo(() => {
    ...
    return blocks
      .map((block, index) => {
        const refId = extractId(block.section) || ''
  ...
  const editBlock = useCallback(
    (refId: string) => {
      router.push(`/admin/collections/sections/${refId}`)
    },
    [router],
  )
    ...
    <div style={{ marginBottom: 24 }}>
      ...
      >
        ...
        {hasBeenActivated && rows.length === 0 && (
          ...
          >
            No sections yet. Create a section for this exercise to add it to the playlist.
          </div>
        )}
