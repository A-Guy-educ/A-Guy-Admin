 ...
 */
function byteEqual(a: ContentBlock[], b: ContentBlock[]): boolean {
  return JSON.stringify(a) === JSON.stringify(b)
}

/**
 * Checks structural equality — same block count, same type at each index,
 * and same key shape when both blocks are plain objects.
 * Returns true if structurally equal.
 */
function blockStructuralEqual(a: ContentBlock[], b: ContentBlock[]): boolean {
 ...
 */
function numericDifferencesOnly(left: unknown, right: unknown): boolean {
 ...
 * 2. blockStructuralEqual(a, b) → 'structural_diff' if block count, type, or key shape differs
