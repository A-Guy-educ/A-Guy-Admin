...
interface StudioDocBlockProps {
  ...
  /**
   * Optional — when provided, a "×" appears on the block chrome (top-right,
   * hover-visible). Parent decides whether to render (typically only when
   * the containing blocks array has more than one entry, so we don't leave
   * the section/exercise with zero blocks — the ContentSchema requires min 1).
   */
  onDelete?: () => void
}
