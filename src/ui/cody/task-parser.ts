 ...
 * @domain cody
 * @pattern task-parser
 * @ai-summary Parse GitHub bot comments to extract Cody task status
 */
...
const RUNNING_STATUS = /🔄 Cody running for `([0-9]{6}-[a-zA-Z0-9-]+)`/
const SUCCESS = /✅ Cody completed for `([0-9]{6}-[a-zA-Z0-9-]+)`/
const FAILURE_CATCH = /❌ Pipeline failed for `([^`]+)`:\s*(.+)$/s
const FAILURE_STATUS = /❌ Cody failed for `([0-9]{6}-[a-zA-Z0-9-]+)`/
const TIMEOUT = /⏰ Cody timed out for `([0-9]{6}-[a-zA-Z0-9-]+)`/
...
/**
 * Parse a single GitHub comment and extract Cody status information
 */
export function parseComment(comment: { body: string; created_at: string }): ParsedComment | null {
  ...
  // Running status: 🔄 Cody running for `260219-auto-98`
  ...
  // Success: ✅ Cody completed for `260219-auto-98`!
  ...
  // Failure (status): ❌ Cody failed for `260219-auto-98`
  ...
  // Timeout: ⏰ Cody timed out for `260219-auto-98`
  ...
  // Clarify stop: 🔄 Cody stopped at clarify stage
...
export function getLatestFailure(comments: ParsedComment[]): ParsedComment | null {
  const failureTypes: CommentType[] = ['failure', 'cody-failed', 'timeout']
