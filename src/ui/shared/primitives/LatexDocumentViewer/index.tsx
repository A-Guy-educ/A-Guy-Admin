...
const LazyAxisRenderer = dynamic(
  () =>
    import('@/ui/shared/exerciserenderer/blocks/AxisRenderer').then((m) => ({
...
const LazyGeometryRenderer = dynamic(
  () =>
    import('@/ui/shared/exerciserenderer/blocks/GeometryRenderer').then((m) => ({
...
export function LatexDocumentViewer({
  ...
  return (
    <div
      dir={dir}
      className={cn(
        'bg-background border-border mx-auto max-w-4xl overflow-auto rounded-lg border shadow-card',
        className,
      )}
    >
      <div className="px-12 py-10 font-serif sm:px-16 sm:py-section-lg">
        {title && (
          <h1
            className={cn(
          ...
          <>
            {segments.map((segment, i) => (
              <React.Fragment key={i}>
                ...
                {i < diagrams.length && (
                  <div className="my-6 rounded-xl border bg-card p-card-padding-sm">
      ...
      {showPrintButton && (
        <div className="fixed bottom-4 right-4 print:hidden">
          <button
            ...
            className={cn(
              'bg-primary text-primary-foreground rounded-md px-4 py-2 text-body-sm',
