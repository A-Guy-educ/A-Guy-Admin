...
// Skip live Atlas tests unless explicitly enabled.
// Vector search requires MongoDB Atlas with search enabled and can exceed the normal gate budget.
const liveVectorSmokeEnabled = process.env.ATLAS_VECTOR_TESTS === '1'
const hasOpenAIKey = !!process.env.OPENAI_API_KEY
const hasAtlasUrl = !!process.env.DATABASE_URL_ATLAS

describe.skipIf(!liveVectorSmokeEnabled || !hasOpenAIKey || !hasAtlasUrl)(
  'Vector Search Validation Integration Tests',
  () => {
    beforeAll(async () => {
